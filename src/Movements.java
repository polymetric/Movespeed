...
public class Movements {
	...
	public static String circle_cw(Machine machine, int div, float speed, float accel) {
		...
		if (machine.X_LEN >= machine.Y_LEN) {
			// These equations were generated by trial and error. I don't think a single
			// person on this planet could tell you
			// what the fuck is going on here.
			output += Gcodes.move_xy(
					machine.X_LEN / 2 - machine.X_LEN / 2 / div + (machine.X_LEN - machine.Y_LEN) / 2 / div,
					machine.Y_LEN / 2);
	...
	public static String circle_ccw(Machine machine, int div, float speed, float accel) {
		...
		if (machine.X_LEN <= machine.Y_LEN) {
			...
			// person on this planet could tell you
			// what the fuck is going on here.
			output += Gcodes.move_xy(machine.X_LEN / 2,
					machine.Y_LEN / 2 - machine.Y_LEN / 2 / div + (machine.Y_LEN - machine.X_LEN) / 2 / div);
